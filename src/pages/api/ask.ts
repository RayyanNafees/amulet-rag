...
export const POST: APIRoute = async ({ request }) => {
	...
	if (data.success === false) {
		return new Response(JSON.stringify({ response: "Invalid request." }), {
			status: 400,
		});
	}
	...
	if (vector_index.get() === undefined) {
		return new Response(JSON.stringify({ error: "Vector index not created" }), {
			status: 500,
		});
	}
	...
	if (!question)
		return new Response(JSON.stringify({ response: "No question provided." }), {
			status: 400,
		});

	if (!qa_chain)
		return new Response(JSON.stringify({ response: "No PDF file uploaded." }), {
			status: 400,
		});
